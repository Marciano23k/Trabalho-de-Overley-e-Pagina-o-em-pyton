...
class Aplicacao:
    ...
    def tela(self):
        self.root.title("Simulador de Overlay e Paginaçao")  # a um titulo para a janela
    ...
    def executar_subrotina_overley(self, subrotina, index):
        ...
        frame_subrotina.place(relwidth=largura_subrotina, relheight=1, relx=posicoes_alinhadas[index], rely=0)
                             
        # cria o titulo da subrotina
        label = Label(frame_subrotina, text=subrotina, font=("Arial", 15), bg=cor)            
        label.pack(expand=True, fill=BOTH)

        # converte o tempo de ms para s
        tempo_restante = self.tempo_execucao[subrotina] / 1000.0
        tempo_label = Label( frame_subrotina, text=f"{tempo_restante:.1f}s", font=("Arial", 15), bg=cor)  
        tempo_label.pack()
        ...
        def atualizar_tempo():
            ...
            if tempo_restante > 0:
                tempo_restante -= 0.1
                self.labels_tempo[subrotina].config(text=f"{tempo_restante:.1f}s")   
                self.root.after(100, atualizar_tempo)
            else:
                self.finalizar_subrotina_overley(subrotina, index, frame_subrotina)
        self.root.after(100, atualizar_tempo)    
    ...
    def executar_subrotina_paginacao(self, subrotina, index):
        ...
        def atualizar_tempo():
            ...
            if tempo_restante > 0:
                ...
                self.finalizar_subrotina_paginacao(subrotina, index, frame_subrotina)
        self.root.after(100, atualizar_tempo)    
    ...
    def atualizar_labels(self):
        ...
        self.label_ativas.config( text="\n".join(label_com_endereco(n) for n in self.subrotinas_ativas.values()))   
        self.label_fila.config(text="\n".join(label_com_endereco(n) for n in self.fila_subrotinas[:10]))
        self.label_concluidas.config( text="\n".join(label_com_endereco(n) for n in self.subrotinas_concluidas))
